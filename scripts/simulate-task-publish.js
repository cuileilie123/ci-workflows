...
// 模拟前端的埋点请求
async function simulateTrackEvent(token) {
  console.log('🚀 开始模拟发布任务场景...');
  ...
  console.log('📊 模拟埋点数据:', JSON.stringify(eventData, null, 2));
  ...
  const options = {
    ...
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`, // 使用获取到的token
      'Content-Length': Buffer.byteLength(postData),
    },
  };
...
async function main() {
  ...
  try {
    ...
    // 发送埋点数据
    await simulateTrackEvent(token);
    console.log('\n🎉 测试完成！');
  } catch (error) {
    console.error('\n💥 测试失败:', error.message);
  }
...
module.exports = { simulateTrackEvent, checkBFFService, getTestToken };